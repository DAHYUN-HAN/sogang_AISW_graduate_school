...
- Completed P0 dues-payment-scope alignment: participant identity no longer comes from member accounts. Admins separately upsert the independent full roster and replace the `ALL` snapshot from headerless `name`, `major`, `student number` XLSX files, or individually assign `ALL`, one active activity-board `ONCE`, or `UNPAID`. Payment uploads match exact name/student number, reset omitted `ALL`, preserve omitted `ONCE`, and reject mismatches atomically. The board-aware member picker searches every roster row by name, exposes only `is_paid_for_board`, renders paid black and unpaid gray, and permits both states to be selected and stored. The API preserves participant-name snapshots and the three-step irreversible-delete flow. Visual verification: `docs/qa/DUES_PAYMENT_SCOPE_VERIFICATION_2026-09-23.md` and `docs/qa/evidence/dues-payment-scope/`.
