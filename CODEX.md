...
- Completed P0 dues-payment-scope alignment: participant identity no longer comes from member accounts. Admins add to the independent full roster from headerless `name`, `major`, `student number` XLSX files; exact existing identities are retained, only new students are added as `UNPAID`, and conflicts reject the import atomically. The separate payment upload replaces the `ALL` snapshot, resetting omitted `ALL` while preserving omitted `ONCE`, and individual editing assigns `ALL`, one active activity-board `ONCE`, or `UNPAID`. Manual payment reset also clears only `ALL`; it never deletes roster rows or `ONCE`. The board-aware member picker searches every roster row by name, exposes only `is_paid_for_board`, renders paid black and unpaid gray, and permits both states to be selected and stored. Visual verification: `docs/qa/DUES_PAYMENT_SCOPE_VERIFICATION_2026-09-23.md` and `docs/qa/evidence/dues-payment-scope/`.
