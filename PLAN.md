...
| Admin page development | Implemented; dues-scope visual QA documented | 2026-06-14 to 2026-06-25 | Admin surface covers launch-critical users, the independent dues-payer roster with additive identity import, replaceable ALL snapshot, ALL-only payment reset, and individual ALL/ONCE/UNPAID editing, notices, posts/comments, reports, FAQs, events, registration settings, and statistics. Roster rows and ONCE assignments survive payment reset. Keep both the protected route and backend admin dependencies. Dues evidence is recorded under `docs/qa/evidence/dues-payment-scope/`. |
...
- IA redesign.
- Quick menu.
- Implemented independent current student roster for subsidy activity-certification participants, with separate admin identity and `ALL` snapshot XLSX flows, individual `ALL`/one-board `ONCE`/`UNPAID` management, permanent-delete safeguards, and board-specific black/gray member presentation. All roster rows remain selectable.
