...
- Activity-certification detail responses include ordered `activity_participants[]` entries with the stored display `label`, roster `id`, and nullable `is_paid_for_board`. Current roster-linked entries are recalculated against the detail board (`ALL` or matching `ONCE` is `true`); legacy name-only snapshots use `id: null` and `is_paid_for_board: null`.
