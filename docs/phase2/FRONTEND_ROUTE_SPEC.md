...
The admin console includes a separate `원우회비` tab. `전체 원우 명부 업로드` atomically upserts identity while preserving status; `전체 납부자 업로드` atomically replaces the `ALL` snapshot by exact name/student-number match while preserving omitted `ONCE` rows. `개별 등록` and edit choose `전체 납부`, `특정 행사 1회 납부` with one active activity-certification board, or `미납`. The paginated list shows the derived scope and current joined board name. It requires `start -> irreversible warning -> exact 진짜 삭제 phrase` before permanent deletion. Member-account cards no longer display or mutate a dues status. Visual evidence: `docs/qa/evidence/dues-payment-scope/admin-bulk-payment.png`, `admin-once-board.png`, and `activity-participant-colors.png`; verification matrix: `docs/qa/DUES_PAYMENT_SCOPE_VERIFICATION_2026-09-23.md`.
