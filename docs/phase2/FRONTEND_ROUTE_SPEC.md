...
| `S_007` | `/events` | Compatibility redirect to Home | user |
...
The admin console includes a separate `원우회비` tab. `전체 원우 명부 업로드` keeps the existing roster, skips exact student-number/identity duplicates, adds only new students as `UNPAID`, and rejects conflicting identity without partial changes. `전체 납부자 업로드` atomically replaces the `ALL` snapshot by exact name/student-number match while preserving omitted `ONCE` rows. `개별 등록` and edit choose `전체 납부`, `특정 행사 1회 납부` with one active activity-certification board, or `미납`; changing an `ONCE` board replaces the previous board assignment. The paginated list shows the derived scope and current joined board name. `원우회비 납부자 초기화` requires the exact `납부자 초기화` phrase and changes only `ALL` rows to `UNPAID`, preserving the roster and `ONCE` rows. Member-account cards no longer display or mutate a dues status. Visual evidence: `docs/qa/evidence/dues-payment-scope/admin-bulk-payment.png`, `admin-once-board.png`, and `activity-participant-colors.png`; verification matrix: `docs/qa/DUES_PAYMENT_SCOPE_VERIFICATION_2026-09-23.md`.

Council content is managed by admins except suggestion and mutual-aid submissions. The current-council route opens its single introduction directly, while cohort leaders and past councils use an admin-created summary list followed by a selected detail screen. All three detail views share an ordered organization-introduction photo gallery, greeting, introduction, and variable-length fixed-profile-card layout. The first `photo_urls[]` image is the representative image and mirrors `banner_image_url`; member rendering resolves `photo_urls[]`, then legacy `attachment_urls[]`, then `banner_image_url`. Admins can select multiple images, add images, delete an individual image, and move each image up or down; saving preserves the resulting order and mirrors its first image. Admins edit the single current-council introduction and can add, edit, reorder through list order, and delete cohort/past organization cards and all member cards; each member includes name, cohort, role, and an optional profile image. Notice create/edit can opt into council activity-history linkage; linked notices reuse their title, body, date, and image attachments in the council list and detail screens.
