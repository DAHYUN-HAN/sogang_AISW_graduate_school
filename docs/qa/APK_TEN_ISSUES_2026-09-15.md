...
- 후속 점검 정정: 사용자가 SG_LLM 대표 이미지의 상세 노출을 지적했다. **8번은 사진 순서만 통과했으며 대표 이미지를 제외해야 한다는 규칙은 실패한다.** 기존 상세 코드에서 대표/상세 구분이 사라진 회귀를 확인했다. [원인 및 영향 범위](PARTICIPATION_REPRESENTATIVE_IMAGE_2026-09-15.md). 아래 9건 통과 수치는 최초 재현 절차의 순서 기준 결과이며, 대표 제외 조건까지 포함한 전체 정상 판정이 아니다.
...
| 8 | 동아리 모집 글 사진 순서 | **순서 통과 / 대표 제외 실패** | 참여활동 → `SG_LLM (LLM구축)`. 이름 → 사진 2장 → 본문 → 가입 신청 순서다. 후속 점검에서 첫 사진은 목록 전용 대표 이미지이므로 상세에서 제외해야 함을 확인했다. | `47-sg-llm`, `48-sg-llm-scroll`, `49-sg-llm-apply-visible`; 후속 원인 문서 |
