...
test("원우회비 화면은 업로드 전체 교체와 개별 행사 1회 납부 등록을 제공한다", () => {
  assert.match(paymentSource, /현재 학기 전체 납부자 업로드/);
  assert.match(paymentSource, /개별 행사 1회 납부 등록/);
  assert.match(paymentSource, /1회 납부 등록/);
  assert.match(paymentSource, /REGISTER_ONCE/);
  ...
  assert.doesNotMatch(paymentSource, /납부자 초기화 시작|resetPayments/);
});
...
test("납부 편집기는 세 가지 납부 범위와 읽기 전용 신원을 제공한다", () => {
  ...
  assert.match(editorSource, /REGISTER_ONCE/);
