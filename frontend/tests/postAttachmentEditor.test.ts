...
function editorHarness(isPrivate = false) {
  ...
  const render = () => {
    cursor = 0;
    return module.exports.default({ attachments: items, isPrivate, onUploadingChange: (value: boolean) => busy.push(value), onChange: (next: any) => { items = typeof next === "function" ? next(items) : next; } });
  };
  ...
  return { press, render, nodes, busy, pickerCalls, opened, items: () => items, select: (value: Item[]) => { result = value; }, fail: () => { failure = true; } };
}
...
test("replacement preserves attachment order and cancellation/failure preserves the old attachment", async () => {
  const h = editorHarness();
  await h.press("photo.png 변경");
  assert.deepEqual(Array.from(h.items(), (item) => item.id), [1, 2]);
  h.select([replacement]);
  await h.press("photo.png 변경");
  assert.deepEqual(Array.from(h.items(), (item) => item.id), [3, 2]);
  h.fail();
